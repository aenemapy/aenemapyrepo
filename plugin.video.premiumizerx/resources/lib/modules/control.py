...
addon_id = 'plugin.video.premiumizerx'
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.premiumizer.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.premiumizer.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.premiumizer.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.premiumizer.artwork)')
...
def moderator():
    netloc = [urlparse.urlparse(sys.argv[0]).netloc, '', 'plugin.video.premiumizerx', 'plugin.video.premiumizerx']


def metaFile():
    if condVisibility('System.HasAddon(script.premiumizer.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.premiumizer.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
