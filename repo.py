...
if (__name__ == "__main__"):
            ...
            if re.search("^(plugin|script|service|skin|repository)" , x) and not re.search('.zip', x):
                ...
                if "addon.xml" in filesinfoldertozip: 
                    ...
                    for elem in root.iter('addon'):
                        ...
                        versionID = str(elem.attrib['version'])
                try:
                    existing_xml = os.path.join(zipsdir, x, 'addon.xml')
    ...
    label = "UPDATED: %s  - SKIPPED: %s" % (str(zipped), str(skipped))
    print(label)
    for z in zippedList: print ("UPDATED: ", z)
